...
class HotZone extends PureComponent {
  ...
  onMouseUp = () => {
    ...
    if (this.width > this.minLength && this.height > this.minLength) {
      this.setState({
        zoneList: [...zoneList, { left: this.x, top: this.y, width: this.width, height: this.height, key: Date.now(), link: '', _blank: false }],
      });
    }
    this.reset();
  };
  ...
  render() {
    ...
    return (
      <div className="cube-hotzone-wrapper">
        ...
        <Modal maskClosable={false} width={800} onOk={this.onOk} onCancel={this.onCancel} visible={visible} className="cube-hotzone-modal">
